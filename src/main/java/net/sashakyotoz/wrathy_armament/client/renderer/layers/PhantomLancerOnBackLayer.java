...
public class PhantomLancerOnBackLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, T pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        if (shouldRender(itemstack, pLivingEntity)) {
            ...
            this.lancerBackModel.renderToBuffer(pPoseStack, pBuffer.getBuffer(RenderType.entityTranslucent(LANCER_LOCATION)), pPackedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
            pPoseStack.popPose();
        }
    }

    public boolean shouldRender(ItemStack stack, T entity) {
        if (entity instanceof Player player && (checkLancer(player) && (!player.getItemInHand(InteractionHand.OFF_HAND).is(WrathyArmamentItems.PHANTOM_LANCER.get())
                && !player.getItemInHand(InteractionHand.MAIN_HAND).is(WrathyArmamentItems.PHANTOM_LANCER.get()))) && (!stack.isEmpty() && Config.SHOW_IF_ARMOR_EQUIP.get() || stack.isEmpty()))
            return Config.SHOW_LANCER_ON_PLAYER_BACK.get();
        else
            return false;
    }

    private boolean checkLancer(Player player) {
        return player.getInventory().hasAnyMatching(stack -> stack.getItem() instanceof PhantomLancer);
    }
}